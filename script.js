...
newElement1.textContent = JSON.stringify(lotrCitiesArray);
...
function vowels() {
var count = 0;
for (i=0; i<gotCitiesCSV.length; i++) {
    if (gotCitiesCSV[i] == 'a' || gotCitiesCSV[i] == 'e' || gotCitiesCSV[i] == 'i' || gotCitiesCSV[i] == 'o' || gotCitiesCSV[i] == 'u') {
      count = count + 1;
      let span19 = document.createElement("span");
      let newElement19 = document.getElementById("div19");
      newElement19.textContent = (count);
...
function EndInOr() {
    ...
    for (i=0; i<lotrCitiesArray.length; i++) {
        if (lotrCitiesArray[i].endsWith('or') === true) {
          ...
          newElement20.textContent = (Orcount);
...
for (i=0; i<bestThing.length;i++) {
    if (bestThing[i].startsWith('b') === true) {
        ...
        newElement21.textContent = (withaB);
